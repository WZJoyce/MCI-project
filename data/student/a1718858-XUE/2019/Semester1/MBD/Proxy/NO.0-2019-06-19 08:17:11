...
while True:
  ...
  try:
    ...
    # ~~~~ INSERT CODE ~~~~
    s = ''.join(outputdata)
    clientRes = 'HTTP/1.1 200 OK\r\n\r\n'+s
    clientSocket.send(clientRes)
    # ~~~~ END CODE INSERT ~~~~
      ...
      try:
        ...
        while True:
          try:
            ...
            tmpRecv = originServerSocket.recv(4096)
            responseFromServer+=tmpRecv
            if (len(tmpRecv)==0):
              break
        ...
        # ~~~~ INSERT CODE ~~~~
        clientSocket.send(responseFromServer)
        # ~~~~ END CODE INSERT ~~~~
        ...
        # ~~~~ INSERT CODE ~~~~
        tmpFile = responseFromServer.split('\r\n')
        position = 0
        while tmpFile[position] != '':
          position+=1
        position+=1
        finalResponseFromServer = '\r\n'.join(tmpFile[position+1:])
        cacheFile.write(finalResponseFromServer)
        # ~~~~ END CODE INSERT ~~~~
